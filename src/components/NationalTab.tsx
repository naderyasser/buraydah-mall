...
export default function NationalTab() {
  const now = new Date();
  ...
  return (
    <Link href="/national-day" className="nd-tab no-print" aria-label="عروض اليوم الوطني 96">
      <span className="nd-tab-num tabular">96</span>
      <span className="nd-tab-txt">{occ ? "عروض اليوم الوطني" : `عروض اليوم الوطني تبدأ بعد ${Math.max(1, Math.ceil(((occasionStart(now)?.getTime() ?? 0) - now.getTime()) / 864e5))} أيام`}</span>
    </Link>
  );
}
